'''*****************************************************************************************
   ** FileName:        Double_Sdes_Attack.py
...
def find(tofind, source):                   #递增
    left = 0
    right = len(source) - 1
    middle = (left + right) >> 1
    while(left <= right):
        if(source[middle] == tofind):
            return True
        elif(source[middle] > tofind):
            right = middle - 1
        else:
            left = middle + 1
        middle = (left + right) >> 1
...
def double_des_attack(k1, k2):
    ...
    for i in range(0, 2 ** 10):
        ...
        if(not find(tmp1, all_index)):
            j = 0
            lengt = len(all_index)
            while( j < lengt and all_index[j] < tmp1):
                j = j + 1
            all_index.insert(j, tmp1)
