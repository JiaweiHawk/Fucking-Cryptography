...
def encode(pipe, message):
    key = pipe.recv()
    message = xor(message, key)
    for i in range(1, 9):
        key = pipe.recv()
        message = col_xor(sub_shift(message), key)
    key = pipe.recv()
    message = xor(sub_shift(message), key)
    print(message)
...
if(__name__ == '__main__'):
    ...
    p1 = multiprocessing.Process(target = keys_generate, args = (pipe[0], key))
    p2 = multiprocessing.Process(target = encode, args = (pipe[1],message))
    start = time.clock()
    p1.start()
    p2.start()
